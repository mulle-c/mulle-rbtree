...
//  mulle-rbtree
//
// The parent P is red but the uncle U is black. The ultimate goal is to
// rotate the parent node P to the grandparent position, but this will
// not work if N is an "inner" grandchild of G (i.e., if N is the left child
// of the right child of G or the right child of the left child of G).
// A dir-rotation at P switches the roles of the current node N and its
// parent P. The rotation adds paths through N  and removes paths through
// P. But both P and N are red, so requirement 4 is preserved.
// Requirement 3 is restored in case 6.
